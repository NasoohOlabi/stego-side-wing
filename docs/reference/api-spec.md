...
- `POST /workflows/research`
  - Body: `count?`, `offset?`, `include_breakdown?` (bool, default `false`)
  - **Default (`include_breakdown: false`):** response `data` is a **list** of researched post objects (same as before).
  - `**include_breakdown: true`:** response `data` is an object:
    - `posts` — same list of researched post objects (including full `search_results` text per post; responses can be very large).
    - `breakdown` — research protocol metadata for benchmarking and tooling:
      - `batch` — `elapsed_ms` (runner wall-clock), `processed_count`, `requested_count`, `offset`, `runner_trace_id`, `preview_total_ms_sum` (sum of per-post `report.timing.preview_total_ms` where present).
      - `posts` — array aligned with `posts`, each `{ "post_id", "report" }` where `report` is the same structure `ResearchPipeline.preview_post` produces (`timing`, `searches`, `fetched_pages`, `terms_report`, `search_results`, skipped-post shape when already researched, etc.).
  - Streaming defaults to SSE; disable via `?stream=0` or `{ "stream": false }`. With streaming, the final `result` event’s `result` field matches the sync `data` shape (list vs `{ posts, breakdown }`).
...
- `POST /workflows/validate-post`
  ...
  - Response `data` shape:
    ...
    - `steps`: object with keys `data_load`, `research`, `gen_angles`, each:
      ...
      - `changed_key_snippets`: when `comparison` is `mismatch`, up to eight `{path, baseline_snippet, rerun_snippet}` rows with capped string/JSON previews at leaves (including `(absent)` when a dict key exists on only one side); empty array when `match` or when not compared
  ...
  - **Mismatch forensics vs logs:** Structured JSONL logs for validate-post include a bounded `failure_detail` (e.g. `sample_changed_keys`, `forensics_note`). For a full picture after `protocol_mismatch` or `rerun_incomplete`, use this endpoint’s JSON body (`steps` per stage) or the streaming client’s final `result` event, which carries the same object—including full `changed_keys` (up to 50 paths), `changed_key_snippets`, `baseline_summary` / `rerun_summary`, and `protocol_report`.
- `POST /workflows/double-process-new-post`
  ...
  - Body: `stream?` (bool; same SSE default as other workflow routes), `allow_angles_fallback?` (bool, default `false`) — passed through to gen-angles (same semantics as `validate-post` / `angles-preview`). Optional `post_id` (non-empty string): run double-process on that id instead of dequeuing from the new-post queue; `source_file` is reported as `{post_id}.json`. If an active `active_post_claim.json` exists for a **different** `post_id`, the API returns **400** until that run finishes or the claim is cleared; if the claim matches the requested `post_id`, the run resumes as usual.
