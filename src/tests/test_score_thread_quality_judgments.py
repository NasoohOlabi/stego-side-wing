...
def _row(post: str, method: str, score: int | None) -> dict[str, object]:
    return {"metric": "thread_relevance", "post_id": post, "method": method, "score": score, "provider": "p", "judge_model": "m", "judge_prompt_sha256": "h"}


def test_summary_pairs_and_clusters_by_post() -> None:
    rows = [_row("a", "our_method", 5), _row("a", "our_method", 1), _row("a", "zlg", 2), _row("b", "our_method", 2), _row("b", "zlg", 4), _row("c", "zlg", 5), _row("d", "our_method", None)]
