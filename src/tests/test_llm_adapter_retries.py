...
def test_retry_uses_provider_requested_delay(monkeypatch: pytest.MonkeyPatch) -> None:
    calls = {"n": 0}
    sleeps: list[float] = []
    ...
    def fake_post(*_args: object, **_kwargs: object) -> MagicMock:
        ...
        if calls["n"] == 1:
            ...
            response.headers = {}
            response.text = "Please retry in 54.5s."
    ...
    monkeypatch.setattr("workflows.adapters.llm.requests.post", fake_post)
    monkeypatch.setattr("workflows.adapters.llm._llm_retry_backoff_sec", lambda _i: 1.0)
    monkeypatch.setattr("workflows.adapters.llm._llm_retry_jitter_sec", lambda _s: 0.0)
    monkeypatch.setattr("workflows.adapters.llm.time.sleep", sleeps.append)

    assert _adapter().call_llm(prompt="hello", provider="lm_studio", model="demo") == "ok"
    assert sleeps == [54.5]
