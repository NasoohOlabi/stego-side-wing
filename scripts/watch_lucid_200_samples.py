#!/usr/bin/env python3
"""Watchdog for a 200-success LUCID sample campaign used for ZLG comparison.
...
_CAMPAIGN = "LUCID_context_weighted_v2_balanced_200"
_TARGET = 200
...
def _write_processes(
    *,
    ...
    lines = [
        ...
        f"Last checked: {_now()}",
        "",
        "## LUCID 200-sample campaign (ZLG comparison)",
        "",
        "| Field | Value |",
        "| --- | --- |",
        f"| Status | **{status}** |",
    ...
    # Preserve note about the prior 100-sample campaign (paused/incomplete).
    prior = _RUNS.glob("LUCID_context_weighted_v2_balanced_100*")
    prior_dirs = sorted(
        p
        for p in prior
        if p.is_dir()
        and (
            p.name == "LUCID_context_weighted_v2_balanced_100"
            or p.name.startswith("LUCID_context_weighted_v2_balanced_100_")
        )
    )
    prior_total = 0
    for d in prior_dirs:
        out = d / "balanced" / "output-results"
        if out.is_dir():
            prior_total += sum(1 for p in out.glob("*.json") if p.is_file())
    daemon_pid_path = _RUNS / "watch_lucid_200_samples.daemon.pid"
    daemon_pid = (
    ...
    lines.extend(
        [
            "",
            "## Prior 100-sample campaign (paused)",
            "",
            f"| Progress | **{prior_total} / 100** across `{len(prior_dirs)}` dirs |",
            "| Note | Left paused so LM Studio is free for the 200-sample run. |",
            "",
            "## Monitor PIDs",
            "",
            "| Role | PID |",
            ...
            "## Watchdog",
            "",
            "- Script: `stego-side-wing/scripts/watch_lucid_200_samples.py`",
