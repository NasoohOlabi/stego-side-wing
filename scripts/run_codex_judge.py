...
from services.codex_judge_client import CodexJudgeConfig, run_codex_judge
...
def _pairs(rows: list[dict[str, Any]]) -> list[dict[str, dict[str, Any]]]:
    grouped: dict[str, dict[str, dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault(str(row.get("pair_id")), {})[str(row.get("method"))] = row
    return [x for _, x in sorted(grouped.items()) if {"our_method", "zlg"}.issubset(x)]


def _tasks(
    metric: str, rows: list[dict[str, Any]], dataset: Path, limit: int | None
) -> list[dict[str, Any]]:
    corpus = []
    ...
    if limit is not None:
        pairs = pairs[:limit]
    for pair in pairs:
        post_id = str(pair["our_method"]["post_id"])
        post, comments = load_post_comments(dataset, post_id)
        cache[post_id] = (post, comments)
        corpus.append(post)
    result = []
    for pair in pairs:
        post, comments = cache[str(pair["our_method"]["post_id"])]
        if metric == "standout" and len(comments) >= 9:
            result.extend(x.model_dump() for x in build_standout(pair, post, comments))
        elif metric == "weak_link" and comments:
            result.append(build_weak_link(pair, post, comments).model_dump())
        elif metric == "suspicion" and comments:
            result.extend(x.model_dump() for x in build_pointwise(metric, pair, post, comments))
        elif metric == "register" and comments:
            result.extend(x.model_dump() for x in build_register(pair, post, comments))
        elif metric == "attribution" and len(corpus) >= 4 and comments:
            result.extend(x.model_dump() for x in build_attribution(pair, post, comments, corpus))
    return result
...
def _run(
    ...
    task_id = hashlib.sha256(
        f"{task['metric']}:{task['pair_id']}:{task['method']}:{prompt_hash}:{schema_hash}:{config.model}:{config.reasoning_effort}".encode()
    ).hexdigest()
...
def _task_id(
    ...
    identity = f"{task['metric']}:{task['pair_id']}:{task['method']}:{prompt_hash}:{schema_hash}:{config.model}:{config.reasoning_effort}"
...
def main() -> int:
    ...
    config = CodexJudgeConfig(model=args.model, reasoning_effort=args.reasoning_effort)
    for task in tasks:
    ...
    done = {x.get("task_id") for x in _load(output)} if output.exists() else set()
