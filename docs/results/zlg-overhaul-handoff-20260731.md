...
**Date:** 2026-07-31
**Status:** Workstreams A–D complete and committed. Workstream E (re-run) is the only
thing left; the server is redeployed and idle, waiting for a smoke run.
...
### 1. Smoke run — 25 samples

```bash
cd d:/Master/code/stego/stego-side-wing
uv run python scripts/run_zlg_batch_comparison.py \
  --source-summary metrics/e2e_runs/scale300_chunk1/balanced/summary.json \
  --server-url http://192.168.100.136:9000 \
  --run-dir metrics/zlg_comparison_runs/zlg_smoke_recalibrated \
  --limit 25
```

Expected, and worth checking before committing to the full run:

- **zero** `harness_extract` failures (was 94/554)
- **zero** `leakage_check` failures would be nice but is not guaranteed — the model is
  Qwen3.5, which emits `<think>` spontaneously. See "Not done" below.
- gate rejections should be dominated by genuine capacity failures, not apostrophes
